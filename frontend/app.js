...
const API_BASE =
  "https://lecture-lens-backend-h2dud6h6e4b8f6d0.eastus2-01.azurewebsites.net";
...
function isValidAudio(file) {
  const okType =
    ALLOWED_TYPES.includes(file.type) || file.name.match(/\.(mp3|wav|m4a)$/i);
...
async function startRecording() {
  ...
  try {
    ...
    if (
      ...
      showError(
        "Could not access microphone. Make sure your device has a working mic.",
      );
    }
    return;
  }
